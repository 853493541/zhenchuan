...
interface Props {
  snapshot: NoWeaponRequiredSnapshot | null;
  loading: boolean;
  errorMessage: string;
  onRetry: () => void;
...
export default function NoWeaponRequiredTab({ snapshot, loading, errorMessage, onRetry, onToggle }: Props) {
  const [search, setSearch] = usePersistentState("abilityEditor.noWeaponRequired.search", "");
  ...
  const undecidedAbilities = filteredAbilities.filter((entry) => !entry.noWeaponRequired && !entry.manuallyExcluded);
  const enabledAbilities = filteredAbilities.filter((entry) => entry.noWeaponRequired);
  ...
  if (loading) {
    return <div className={styles.statePanel}>正在加载无需武器技能列表…</div>;
  }
  ...
  return (
    <div style={{ maxWidth: 1260, margin: "0 auto", paddingTop: 16 }}>
      ...
      >
        <div>
          <SectionHeader
            title={`已声明仍需武器 (${excludedAbilities.length})`}
            background="#fdf1f1"
            color="#8a4040"
            border="#e5b1b1"
          />
        ...
        <div>
          <SectionHeader
            title={`无需武器 (${enabledAbilities.length})`}
            background="#eef5ff"
            color="#264a7a"
            border="#9ec1f0"
          />
      ...
      <p style={{ marginTop: 16, fontSize: 12, color: "#8a7f6f" }}>
        这里决定哪些技能拥有“无需武器”属性。右侧技能在被缴械时仍可施放，中间列表可以直接判定，左侧“恢复”会清除手动覆盖并回到代码默认值。
      </p>
    </div>
  );
}
...
function AbilityHeading({ entry }: { entry: NoWeaponRequiredEntry }) {
  return (
    <>
