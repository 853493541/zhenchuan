...
export default function CooldownReviewTab({
  entries,
  loading,
  errorMessage,
  onRetry,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginTop: 16, alignItems: "start" }}>
        <Column title={`需要补充 (${needsMore.length})`} background="#fff7e8" color="#9a5b16" border="#e7bc75">
          {needsMore.length === 0 ? <EmptyState text="当前没有需要补充的条目" /> : needsMore.map((entry) => (
            <CooldownRow key={entry.id} entry={entry} disabled={busyKey === entry.id} onStatus={(status, cooldownText) => handleStatus(entry, status, cooldownText)} actions={[
...
function CooldownRow({
  ...
  entry: AbilityCooldownReviewEntry;
  disabled: boolean;
  onStatus: (status: DescriptionReviewStatus, cooldownSecondsText: string) => Promise<void>;
  actions: RowAction[];
}) {
    ...
    <div style={{ padding: "10px 12px", borderBottom: "1px solid #f0ece6" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
        ...
        <input
          ...
          inputMode="decimal"
          value={draft}
          onChange={(event) => setDraft(filterDecimalInput(event.target.value))}
          disabled={disabled}
          style={{
            width: 92,
            minHeight: 30,
            ...
            background: disabled ? "#f0ece6" : "#fffdf9",
            color: "#211d18",
            fontSize: 13,
            fontWeight: 800,
            padding: "4px 8px",
        ...
        />
        <span style={{ fontSize: 12, color: "#7a6e62" }}>秒</span>
      </div>
