...
interface Props {
  snapshot: BuffTimerVisibilitySnapshot | null;
  loading: boolean;
  errorMessage: string;
  onRetry: () => void;
  onToggle: (buffId: number, mode: BuffTimerVisibilityMode) => Promise<void>;
}

export default function BuffTimerVisibilityTab({ snapshot, loading, errorMessage, onRetry, onToggle }: Props) {
  const [search, setSearch] = usePersistentState("abilityEditor.buffTimerVisibility.search", "");
  ...
  const hiddenTimerBuffs = filteredBuffs.filter((entry) => entry.manuallyHidden);
  const undecidedBuffs = filteredBuffs.filter((entry) => !entry.manuallyHidden && !entry.manuallyVisible);
  const visibleTimerBuffs = filteredBuffs.filter((entry) => entry.manuallyVisible);

  async function handleToggle(buffId: number, mode: BuffTimerVisibilityMode) {
  ...
  if (loading) {
    return <div className={styles.statePanel}>正在加载时间显示配置…</div>;
  }
  ...
  return (
    <div style={{ maxWidth: 1260, margin: "0 auto", paddingTop: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginTop: 16, alignItems: "start" }}>
        <div>
          <SectionHeader title={`不显示时间 (${hiddenTimerBuffs.length})`} background="#fdf1f1" color="#8a4040" border="#e5b1b1" />
          <ListShell border="#e5b1b1" background="#fff8f8">
            {hiddenTimerBuffs.length === 0 ? <EmptyState text="当前没有不显示时间的 Buff" /> : hiddenTimerBuffs.map((entry) => (
        ...
        <div>
          <SectionHeader title={`显示时间 (${visibleTimerBuffs.length})`} background="#eef5ff" color="#264a7a" border="#9ec1f0" />
          <ListShell border="#9ec1f0" background="#f8fbff">
            {visibleTimerBuffs.length === 0 ? <EmptyState text="当前没有显式显示时间的 Buff" /> : visibleTimerBuffs.map((entry) => (
              <BuffRow key={entry.buffId} entry={entry} disabled={togglingBuffId === entry.buffId} actionLabel="改为不显" actionTint="#264a7a" onAction={() => handleToggle(entry.buffId, "manual-exclude")} />
            ))}
          </ListShell>
        </div>
      </div>
...
function DecisionBuffRow({ entry, disabled, onDecideNo, onDecideCan }: { entry: BuffTimerVisibilityEntry; disabled: boolean; onDecideNo: () => void; onDecideCan: () => void }) {
  return (
...
function BuffRow({ entry, disabled, actionLabel, actionTint, onAction }: { entry: BuffTimerVisibilityEntry; disabled: boolean; actionLabel: string; actionTint: string; onAction: () => void }) {
  return (
    <BuffBaseRow entry={entry}>
...
function BuffBaseRow({ entry, children }: { entry: BuffTimerVisibilityEntry; children: React.ReactNode }) {
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderBottom: "1px solid #f0ece6" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          ...
          {entry.manuallyVisible && <Badge tint="#315c9f" text="显示时间" />}
          {entry.manuallyHidden && <Badge tint="#b04444" text="不显示时间" />}
        </div>
