...
interface Props {
  snapshot: ManualCancelableBuffSnapshot | null;
  loading: boolean;
  errorMessage: string;
  onRetry: () => void;
  onToggle: (buffId: number, mode: ManualCancelableBuffMode) => Promise<void>;
}

export default function ManualCancelableBuffTab({ snapshot, loading, errorMessage, onRetry, onToggle }: Props) {
  const [search, setSearch] = usePersistentState("abilityEditor.manualCancelableBuffs.search", "");
  ...
  const excludedBuffs = filteredBuffs.filter((entry) => entry.manuallyExcluded && !entry.canManualCancel);
  const undecidedBuffs = filteredBuffs.filter((entry) => !entry.canManualCancel && !entry.manuallyExcluded);
  const cancelableBuffs = filteredBuffs.filter((entry) => entry.canManualCancel);

  async function handleToggle(buffId: number, mode: ManualCancelableBuffMode) {
  ...
  if (loading) {
    return <div className={styles.statePanel}>正在加载主动取消 Buff 配置…</div>;
  }
  ...
  return (
    <div style={{ maxWidth: 1260, margin: "0 auto", paddingTop: 16 }}>
      <div className={styles.rarityFilterBar}>
        <div className={styles.tagFilterRow}>
          <input
            ...
            placeholder="搜索 Buff 名称、ID、来源或描述"
            style={{ maxWidth: 340 }}
          />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginTop: 16, alignItems: "start" }}>
        <div>
          <SectionHeader title={`不可主动取消 (${excludedBuffs.length})`} background="#fdf1f1" color="#8a4040" border="#e5b1b1" />
          <ListShell border="#e5b1b1" background="#fff8f8">
            {excludedBuffs.length === 0 ? <EmptyState text="当前没有不可主动取消的 Buff" /> : excludedBuffs.map((entry) => (
        ...
        <div>
          <SectionHeader title={`可以主动取消 (${cancelableBuffs.length})`} background="#eef5ff" color="#264a7a" border="#9ec1f0" />
          <ListShell border="#9ec1f0" background="#f8fbff">
            {cancelableBuffs.length === 0 ? <EmptyState text="当前没有可以主动取消的 Buff" /> : cancelableBuffs.map((entry) => (
              <BuffRow key={entry.buffId} entry={entry} disabled={togglingBuffId === entry.buffId} actionLabel="改为不可" actionTint="#264a7a" onAction={() => handleToggle(entry.buffId, "manual-exclude")} />
            ))}
          </ListShell>
        </div>
      </div>
...
function DecisionBuffRow({ entry, disabled, onDecideNo, onDecideCan }: { entry: ManualCancelableBuffEntry; disabled: boolean; onDecideNo: () => void; onDecideCan: () => void }) {
  return (
    <BuffBaseRow entry={entry}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
        <SmallActionButton disabled={disabled} tint="#9a4a2a" onClick={onDecideNo}>X</SmallActionButton>
        <SmallActionButton disabled={disabled} tint="#305f3d" onClick={onDecideCan}>✓</SmallActionButton>
      </div>
    </BuffBaseRow>
  );
}

function BuffRow({ entry, disabled, actionLabel, actionTint, onAction }: { entry: ManualCancelableBuffEntry; disabled: boolean; actionLabel: string; actionTint: string; onAction: () => void }) {
  return (
    <BuffBaseRow entry={entry}>
...
function BuffBaseRow({ entry, children }: { entry: ManualCancelableBuffEntry; children: React.ReactNode }) {
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderBottom: "1px solid #f0ece6" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          ...
          {entry.canManualCancel && <Badge tint="#315c9f" text="可以主动取消" />}
          {entry.manuallyExcluded && <Badge tint="#b04444" text="不可主动取消" />}
        </div>
...
function SmallActionButton({ disabled, tint, onClick, children, wide = false }: { disabled: boolean; tint: string; onClick: () => void; children: React.ReactNode; wide?: boolean }) {
  return (
    <button type="button" disabled={disabled} onClick={onClick} style={{ width: wide ? 72 : 34, minHeight: 28, borderRadius: 6, border: `1px solid ${tint}`, background: disabled ? "#ddd" : "transparent", color: disabled ? "#777" : tint, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer" }}>
      {children}
    </button>
  );
}
