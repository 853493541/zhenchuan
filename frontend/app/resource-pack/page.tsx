...
import { CheckCircle2, Download, RotateCcw, Trash2, X } from "lucide-react";
...
export default function ResourcePackPage() {
  ...
  const progressPct = toPercent(completed, assets.length);
  ...
  const etaText = status === "downloading" && speedBytesPerSecond > 0
    ...
    : "计算中";
  const primaryDownloadLabel = status === "downloading"
    ? "暂停下载"
    : status === "done"
      ? "重新下载资源包"
      : cachedCount > 0 && cachedCount < assets.length
        ? "继续下载"
        : "下载资源包";
  ...
  useEffect(() => {
    ...
    if (status !== "done" && status !== "ready" && status !== "failed") return;
    const ready = status === "done" && assets.length > 0 && cachedCount === assets.length;
    window.parent.postMessage({ type: "resource-pack-gate", ready }, window.location.origin);
  }, [assets.length, cachedCount, embedded, status]);
  ...
  const downloadPack = useCallback(async () => {
    ...
    try {
      ...
      let cached = 0;
      let processed = 0;
      const pendingAssets: ResourceAsset[] = [];
      const failed: string[] = [];
      ...
      await runWithConcurrency(nextAssets, 12, async (asset) => {
        ...
        if (match) {
          cached += 1;
          processed += Number.isFinite(asset.size) && asset.size > 0 ? asset.size : 0;
          setCachedCount(cached);
      ...
      await runWithConcurrency(pendingAssets, DOWNLOAD_CONCURRENCY, async (asset) => {
      ...
      if (controller.signal.aborted && downloadPausedRef.current) {
        setStatus("ready");
        await refreshStorage();
        return;
      }
  ...
  }, [assets, loadPackAssets, refreshStorage, status]);
  ...
  useEffect(() => {
    ...
    if (action === "check") {
      const readyFlag = (() => {
        try {
          return window.localStorage.getItem(READY_STORAGE_KEY) === "true";
        } catch {
          return false;
        }
      })();
  ...
  const showInlineProgress = assets.length > 0;
  const inlineProgressLabel = status === "importing"
    ? "导入进度"
    : (status === "checking" ? "校验进度" : "下载进度");
  ...
  return (
    <main className={`${styles.page} ${embedded ? styles.pageEmbedded : ""}`}>
      ...
      <section className={styles.progressSection}>
        ...
        {showInlineProgress && (
          <>
            <div className={styles.progressTrack} aria-label={inlineProgressLabel}>
