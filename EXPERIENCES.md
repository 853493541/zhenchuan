...
- **Mobility/grounding sync**: For channel skills, "可移动读条" and "不可在空中释放" are controlled by different fields and must be set together. Use `channelCancelOnMove: false` for movable channeling, `requiresGrounded: true` for ground-only cast, and clear `requiresStanding` when movement-cast should be allowed.
- **风流云散 Z-preserve split**: Distinguish between active cast dash and 云散 auto-followup blink. Keep `groundTarget.z` vertical targeting for the active `GROUND_TARGET_DASH` cast, but preserve current Z in auto-followup reposition (`yunSan.ts`) so auto dashes only shift XY.
